...
export const clipboardApi = {
  ...
  startMonitoring: (): Promise<void> => 
    invoke('start_clipboard_monitoring'),
  ...
  checkDuplicate: (content: string): Promise<boolean> => 
    invoke('check_duplicate_content', { content }),

  // 監視サービス
  startMonitoringService: (): Promise<void> => 
    invoke('start_clipboard_monitoring_service'),

  stopMonitoringService: (): Promise<void> => 
    invoke('stop_clipboard_monitoring_service'),

  getMonitoringStatus: (): Promise<boolean> => 
    invoke('get_clipboard_monitoring_status'),
};
...
export const historyApi = {
  ...
  // 古いアイテムクリーンアップ
  cleanupOldItems: (maxItems: number): Promise<void> => 
    invoke('cleanup_old_items', { maxItems }),
};
...
export const exportApi = {
  ...
  exportJson: (): Promise<string> => 
    invoke('export_clipboard_history_json'),

  // CSV エクスポート
  exportCsv: (): Promise<string> => 
    invoke('export_clipboard_history_csv'),

  // JSON インポート
  importJson: (jsonData: string): Promise<number> => 
    invoke('import_clipboard_history_json', { jsonData }),
  ...
  // ファイル読み込み
  loadImportFile: (filePath: string): Promise<string> => 
    invoke('load_import_file', { filePath }),

  // エクスポート形式取得
  getExportFormats: (): Promise<ExportFormat[]> => 
    invoke('get_export_formats'),
};
