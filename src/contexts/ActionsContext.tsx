...
const defaultActions: GlobalAction[] = [
  ...
  // 変換・計算
  {
    id: 'calculate',
    label: '計算実行',
    command: 'try { const result = eval(CONTENT); navigator.clipboard.writeText(String(result)); } catch(e) { alert("計算エラー"); }',
    icon: 'Calculator',
    enabled: true,
    priority: 26,
  ...
  // ファイル関連
  {
    id: 'save-file',
    label: 'ファイルに保存',
    command: 'const blob = new Blob([CONTENT], {type: "text/plain"}); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "clipboard.txt"; a.click();',
    icon: 'Folder',
    enabled: true,
    priority: 36,
